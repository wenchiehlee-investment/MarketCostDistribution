...
# -*- coding: utf-8 -*-
"""
taishin_batch_intraday.py — 專注名單批次入口（薄包裝）
...
預設等同於：
    python skills/skill-market-cost-distribution/scripts/run_market_cost.py \
        --list StockID_TWSE_TPEX_focus.csv --data-dir data --output-dir output

額外參數會原樣轉傳給技能 runner（例如 --offline）。
"""
import runpy
import sys
...
def main():
    ...
    if not any(a in ("--list", "--symbol") for a in passthrough):
        argv += ["--list", str(REPO_ROOT / "StockID_TWSE_TPEX_focus.csv")]
